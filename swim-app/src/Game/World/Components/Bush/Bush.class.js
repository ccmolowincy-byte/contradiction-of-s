...
export default class Bush {
  constructor() {
    ...
    this.composerColorOverride = null;
  ...
  getColorsForBushType(bushType, envTime) {
    if (this.composerColorOverride) {
      return {
        shadowColor: this.composerColorOverride.shadowColor.clone(),
        midColor: this.composerColorOverride.midColor.clone(),
        highlightColor: this.composerColorOverride.highlightColor.clone(),
      };
    }
  ...
  init() {
    ...
    this.layoutDefinitions = clonePlain(this.BUSH_DEFINITIONS);
  ...
  spawnFromDefinitions() {
    ...
    const definitions = this.layoutDefinitions || this.BUSH_DEFINITIONS;
  ...
  updateColors() {
    if (this.composerColorOverride) {
      this.applyComposerColorsToMaterial();
      this.bushManager?.setAllColors?.(this.composerColorOverride);
      return;
    }
  ...
  applyComposerColorsToMaterial() {
    if (!this.material || !this.composerColorOverride) return;

    this.material.uniforms.uShadowColor.value.copy(
      this.composerColorOverride.shadowColor
    );
    this.material.uniforms.uMidColor.value.copy(
      this.composerColorOverride.midColor
    );
    this.material.uniforms.uHighlightColor.value.copy(
      this.composerColorOverride.highlightColor
    );
  }

  setComposerColors(colors = {}) {
    const current = this.composerColorOverride || {
      shadowColor: this.material.uniforms.uShadowColor.value,
      midColor: this.material.uniforms.uMidColor.value,
      highlightColor: this.material.uniforms.uHighlightColor.value,
    };

    this.composerColorOverride = {
      shadowColor: normalizeColor(colors.shadowColor) || current.shadowColor.clone(),
      midColor: normalizeColor(colors.midColor) || current.midColor.clone(),
      highlightColor:
        normalizeColor(colors.highlightColor) || current.highlightColor.clone(),
    };
  ...
  /**
   * Move a single canopy bush (tree/birch leaves) to an absolute world position.
   * Index matches the order in layoutDefinitions.
   */
  setCanopyPosition(index, position) {
    if (!this.bushManager || !this.bushManager.bushes[index]) return;
    this.bushManager.updateBushPosition(index, position);
  }

  /**
   * Move several canopy bushes at once.
   */
  setCanopyPositions(indices, position) {
    if (!this.bushManager) return;
    indices.forEach((index) => this.setCanopyPosition(index, position));
  }
