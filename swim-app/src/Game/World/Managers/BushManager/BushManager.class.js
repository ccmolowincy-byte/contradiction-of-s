...
`;
  ...
  addBush({
    ...
    if (randomSeed !== null) {
      ...
      sampler = new MeshSurfaceSampler(this.samplerMesh)
        ...
        .build();
      random = mulberry32(randomSeed + 1013904223);
    }
    ...
    for (let i = 0; i < leafCount; i++) {
      ...
      const samplePosition = positionLocal
        .clone()
        .multiplyScalar(distributionScale)
        .applyQuaternion(rotation)
        .add(position);
      const sampleNormal = normal.clone().applyQuaternion(rotation).normalize();

      dummy.position.copy(samplePosition);
      ...
      this.instanceNormals[instanceIndex * 3 + 0] = sampleNormal.x;
      this.instanceNormals[instanceIndex * 3 + 1] = sampleNormal.y;
      this.instanceNormals[instanceIndex * 3 + 2] = sampleNormal.z;
  ...
  setAllColors({
    ...
    const applyColor = (target, color) => {
      if (!color) return;
      for (let i = 0; i < this.currentLeafIndex; i++) {
        target[i * 3 + 0] = color.r;
        target[i * 3 + 1] = color.g;
        target[i * 3 + 2] = color.b;
      }
    };

    applyColor(this.instanceShadowColors, shadowColor);
    applyColor(this.instanceMidColors, midColor);
    applyColor(this.instanceHighlightColors, highlightColor);
    applyColor(this.instanceColorMultiplier, colorMultiplier);

    const geometry = this.instancedMesh.geometry;
    if (geometry.attributes.instanceShadowColor) {
      geometry.attributes.instanceShadowColor.needsUpdate = true;
    }
    if (geometry.attributes.instanceMidColor) {
      geometry.attributes.instanceMidColor.needsUpdate = true;
    }
    if (geometry.attributes.instanceHighlightColor) {
      geometry.attributes.instanceHighlightColor.needsUpdate = true;
    }
    if (geometry.attributes.instanceColorMultiplier) {
      geometry.attributes.instanceColorMultiplier.needsUpdate = true;
    }
  }
