...
// Asset paths in config/assets.js are authored absolute-from-root ('/models/…').
// When this app is served from a sub-path (Vite base '/swim/'), those must be
// prefixed with the deploy base, since three.js loaders fetch raw runtime strings
// that Vite's build does NOT rewrite. BASE_URL is '/swim/' in this build, '/' at root.
const BASE_URL = import.meta.env.BASE_URL || '/';
function withBase(p) {
  if (typeof p !== 'string') return p;
  return p.charAt(0) === '/' ? BASE_URL + p.slice(1) : p;
}
function resolvePath(path) {
  return Array.isArray(path) ? path.map(withBase) : withBase(path);
}
...
export default class ResourceLoader extends EventEmitter {
  constructor(assets, isDebugMode) {
    ...
    this.sources.forEach((src) => {
      const paths = (Array.isArray(src.path) ? src.path : [src.path]).map(
        withBase
      );
      paths.forEach((url) => {
