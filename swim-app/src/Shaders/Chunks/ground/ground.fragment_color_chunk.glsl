...
vec3 proceduralColor = mix(uGroundColorLight, uGroundColorDark, heightMap.r);
proceduralColor = mix(proceduralColor, uGroundColorBelowGrass, densityMask);

vec3 rockColor = rockMap.rgb * uRockColor * mix(1.0, rockAO, 2.0 * rockMask);
vec3 proceduralGround = mix(rockColor, proceduralColor, rockMask);

vec3 texturedGround = texture2D(uGroundTextureMap, vUv).rgb;
vec3 groundColor = mix(proceduralGround, texturedGround, uUseGroundTexture);

float waterMask = densityMap.b * uShowLegacyLake;
