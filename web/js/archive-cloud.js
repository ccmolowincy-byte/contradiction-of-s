...
import { loadCustomSkel } from './custom-skel-draw.js?v=18';

const GOLDEN   = Math.PI * (3 - Math.sqrt(5)); // ~137.5 deg, sunflower angle
const MAX_R    = 2.8;
const MAX_TRAC = 200;
...
function _hexToPetal(hex) {
  const m = /^#?([0-9a-fA-F]{6})$/.exec((hex || '').trim());
  if (!m) return 'rgba(185,140,50,1)';
  const n = parseInt(m[1], 16);
  return 'rgba(' + ((n >> 16) & 255) + ',' + ((n >> 8) & 255) + ',' + (n & 255) + ',1)';
}

function _tracePalette(trace) {
  // A visitor-chosen colour (from the star wheel) wins over the hashed default.
  const c = trace && trace.visual_params && trace.visual_params.colour;
  if (c) return { bone: c, petal: _hexToPetal(c) };
...
export async function initGarden(canvas, options = {}) {
  ...
  const dprCap = lowPowerGarden ? 0.7 : mobileGarden && !highGraphics ? 0.85 : 2.0;
  ...
  /* -- Sanctuary mini-planet - the dome the constellation figures seat upon ----
   ...
   * figure code, so a slow/failed GLB never breaks the garden (gallery Wi-Fi
   * robustness) - figures just arc on the implied dome. The mesh writes depth,
  ...
  */
  ...
  let _targetCamZ = 5.5;
  const _CAM_Z_MIN = 2.5, _CAM_Z_MAX = 13.0;
   ...
   * ----------------------------------------------------------------------- */
  const ANIM_SLOTS   = lowPowerGarden ? 2 : mobileGarden && !highGraphics ? 4 : 28;  // most-recent N traces: full pose cycling + live petals
  const BREATHE_SLOTS = lowPowerGarden ? 6 : mobileGarden && !highGraphics ? 12 : 90; // next traces: petal breathing only (fixed mid-frame, live clock)
  ...
  /* -- Soft radial vignette - fades petal/edge clipping gracefully --------- */
  ...
  function _buildSkeletonTrace(trace, frames, palette, traceScale) {
    ...
    // -- Root-anchor: average body position across ALL animation frames so the
    // figure stays centred on canvas throughout playback.
    // Previously only frame 0 was used - any frame where the body had shifted
    ...
    let fitScale = 1.0, pivotNX = 0.50, pivotNY = 0.60;
    {
      ...
      renderFrames.forEach(frame => {
        ...
        // Include the automatic stylized head placeholder.
        // This mirrors estimateHead() in custom-skel-draw.js.
        ...
        if (lSh && rSh && lSh.s > 0.10 && rSh.s > 0.10) {
          const sPx  = Math.max(34, Math.hypot(rSh.x - lSh.x, rSh.y - lSh.y));
          const midX = (lSh.x + rSh.x) / 2;
          const midY = (lSh.y + rSh.y) / 2;
          const nose = _hasLandmarks(frame) ? pts[MP.nose] : pts[LEGACY.nose];
          const rx   = Math.max(24, sPx * 0.52);
          const ry   = Math.max(30, sPx * 0.62);
          let hx = midX;
          let hy = midY - sPx * 0.82;

          if (nose && nose.s > 0.18) {
            const nx = Math.max(midX - sPx * 0.34, Math.min(midX + sPx * 0.34, nose.x));
            const ny = Math.max(midY - sPx * 1.14, Math.min(midY - sPx * 0.38, nose.y - sPx * 0.18));
            hx = midX + (nx - midX) * 0.22;
            hy = hy + (ny - hy) * 0.18;
          }

          const pad = Math.max(4, sPx * 0.05);
          if (hx - rx - pad < bMinX) bMinX = hx - rx - pad;
          if (hx + rx + pad > bMaxX) bMaxX = hx + rx + pad;
          if (hy - ry - pad < bMinY) bMinY = hy - ry - pad;
          if (hy + ry + pad > bMaxY) bMaxY = hy + ry + pad;
        }
      });
    ...
    if (customSkel) {
      ...
      const labelText = (trace.visual_params && trace.visual_params.colour)
        ? String(trace.visual_params.colour).toUpperCase()
        : (trace.id || '???').slice(0, 3).toUpperCase();
  ...
  function _layout(instant = false) {
    ...
    ribbons.forEach((r, i) => {
      ...
      if (PLANET.enabled) {
        ...
        const seatR   = PLANET.radius;
        const rho     = Math.min(radius, seatR);
        const sphereY = Math.sqrt(Math.max(0, seatR * seatR - rho * rho));
  ...
  function loadBatch(traces) {
    ...
    _layout(true);   // instant positioning - no drift on initial load
    ...
    const N = ribbons.length;
    const autoZ = Math.max(3.4, Math.min(4.4, 3.1 + Math.sqrt(N) * 0.3));
    _targetCamZ = autoZ;
